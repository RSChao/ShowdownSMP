...
public class events implements Listener {
   ...
   public void onPlayerDeath(PlayerDeathEvent ev){
      ...
      if(p == null) return;
      if(!plugin.getConfig().getBoolean("lives.enabled")) return;
         ...
         if(p.getInventory().getItemInMainHand().hasItemMeta()){
            if(p.getInventory().getItemInMainHand().getItemMeta().getPersistentDataContainer().has(new NamespacedKey(Plugin.getPlugin(Plugin.class), "emblem"))){
               ...
               if(i<odds && (dmg < 400 || p.hasPermission("totem.nolimit"))){
                  ev.setCancelled(true);
                  ShowdownScript<?> scr = ScriptRegistry.getScript("bossfight:pop_player");
                  scr.setArgs(p);
                    scr.run();
   ...
   @EventHandler
    void onPlayerChat(AsyncPlayerChatEvent ev) {
        Player p = ev.getPlayer();
        if(ev.getMessage().startsWith("!banner")){
            ev.setCancelled(true);
            String[] parts = ev.getMessage().split(" ", 2);
            if(parts.length < 2){
                p.sendMessage(ChatColor.RED + "Usage: !banner <message>");
                return;
            }
            ItemStack item;
            switch(parts[1]){
